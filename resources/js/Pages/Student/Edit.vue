<template>
    <div>
        <Card class="max-w-xl">
            ...
            <template #content>
               <form @submit.prevent="updateStudent" method="post" class=" h-auto flex flex-col gap-4">
...
Button
    ...
    setup(props) {
        const {id, nama, alamat, no_telp, fakultas_id, program_studi_id} = props.student;
        const form = useForm({
            nama: nama,
            alamat: alamat,
            no_telp: no_telp,
        ...
        const updateStudent = () => {
            form.patch(route('student.update', id));
        }
